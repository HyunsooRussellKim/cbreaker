...
public class CitusDialectPrepared extends PreparedStatementSQLDialect {
    CitusSQLDialect delegate;
    
    public CitusDialectPrepared(JDBCDataStore dataStore) {
        super( dataStore );
        delegate = new CitusSQLDialect( dataStore );
    }
   ...
   @Override
    ...
    public void setGeometryValue(Geometry g, int dimension, int srid, Class binding,
        ...
        if ( g != null ) 
        {
        	byte[] buffer = new WKBWriter(dimension).write( g );
            ps.setBytes( column,  buffer);
