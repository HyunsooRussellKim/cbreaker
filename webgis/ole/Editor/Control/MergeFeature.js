...
OpenLayers.Editor.Control.MergeFeature = OpenLayers.Class(OpenLayers.Control.Button, {
    ...
    initialize: function(layer, options) {
        ...
        this.events.register('activate', this, this.mergeFeature);
        ...
        if (this.layer.selectedFeatures.length < 2) {
            ...
            citus.task(function() {            
                ...
                OpenLayers.Request.POST({
                    url: this.map.editor.oleMergeUrl,
                    data: OpenLayers.Util.getParameterString({geo: geo, tableName: this.map.editor.tableName}),
                    headers: {"Content-Type": "application/x-www-form-urlencoded"},
                    callback: this.map.editor.requestMergeComplete,
                    proxy: this.proxy,
