    ...
    drawFeature: function(geometry) {
        ...
        if (proceed !== false) {
            if (this.layer.selectedFeatures.length > 0) {
                citus.task(function() {
                    var geo = wktFormat.write(this.layer.selectedFeatures[0]),
                        cut = wktFormat.write(feature);
                    this.map.editor.startWaiting(this.panel_div);
                    OpenLayers.Request.POST({
                        url: this.map.editor.oleSplitUrl,
                        data: OpenLayers.Util.getParameterString({cut: cut, geo: geo, tableName: this.map.editor.tableName}),
                        headers: {"Content-Type": "application/x-www-form-urlencoded"},
                        callback: this.map.editor.requestSplitComplete,
                        proxy: this.proxy,
